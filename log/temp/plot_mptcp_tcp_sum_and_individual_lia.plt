set terminal png
set output 'MPTCP_Throughput_vs_Time_lia.png'
#set xdata time
#set timefmt "%S"
set xlabel "time"
...
set grid
f(x) = mean_1
fit f(x) 'mptcp_on_w_e_ethr_lia.txt' via mean_1
f(x) = mean_2
fit f(x) 'mptcp_on_w_e_wifi_lia.txt' via mean_2
f(x) = mean_3
fit f(x) 'mptcp_on_w_e_sum_lia.txt' via mean_3
...
plot "mptcp_on_w_e_wifi_lia.txt" with line title "WIFI", \
"mptcp_on_w_e_ethr_lia.txt" with line title "ETHR", \
 "mptcp_on_w_e_sum_lia.txt" with line title "WIFI+ETHR" 
